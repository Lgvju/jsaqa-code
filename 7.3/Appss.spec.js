...
test("Failed authorization", async () => {
  ...
  await page.screenshot({ path: "screenshotFailedautorization.png", fullPage: true });
  // browser.close();
}, 500);
...
test("Successful authorization", async () => {
  ...
  //await expect(page.locator("h2")).toContainText("Moё обучение");
  await page.getByText('Moё обучение');
  await page.screenshot({ path: "screenshotSuccessfulauto.png", fullPage: true });
  browser.close(); 
}, 60000);